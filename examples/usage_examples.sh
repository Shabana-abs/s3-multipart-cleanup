...
python scripts/s3_multipart_cleanup_manager_v2.py --dry-run --days 7

# Apply to all buckets with 7-day cleanup
python scripts/s3_multipart_cleanup_manager_v2.py --days 7 --apply

# FILTERED USAGE
echo "=== Filtered Usage Examples ==="

# Filter by bucket pattern (regex)
python scripts/s3_multipart_cleanup_manager_v2.py --bucket-pattern "myapp-.*" --days 7 --apply

# Filter by region
python scripts/s3_multipart_cleanup_manager_v2.py --region us-west-2 --days 7 --apply

# Combine filters
python scripts/s3_multipart_cleanup_manager_v2.py --bucket-pattern "prod-.*" --region us-east-1 --days 7 --apply

# PROGRESSIVE ROLLOUT EXAMPLES
echo "=== Progressive Rollout Strategy ==="

# Stage 1: Test on dev buckets
python scripts/s3_multipart_cleanup_manager_v2.py --bucket-pattern ".*-dev-.*" --days 7 --apply

# Stage 2: Staging environment  
python scripts/s3_multipart_cleanup_manager_v2.py --bucket-pattern ".*-staging-.*" --days 7 --apply

# Stage 3: Production by region
python scripts/s3_multipart_cleanup_manager_v2.py --region us-west-2 --days 7 --apply
python scripts/s3_multipart_cleanup_manager_v2.py --region us-east-1 --days 7 --apply
python scripts/s3_multipart_cleanup_manager_v2.py --region eu-west-1 --days 7 --apply
...
python scripts/s3_multipart_cleanup_manager_v2.py --days 1 --apply

# Conservative cleanup with limited concurrency
python scripts/s3_multipart_cleanup_manager_v2.py --days 7 --max-workers 2 --apply

# Use specific AWS profile
python scripts/s3_multipart_cleanup_manager_v2.py --profile production --days 7 --apply
